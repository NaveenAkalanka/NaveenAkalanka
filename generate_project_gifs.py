...
# Terminal dimensions (narrower to fit inside a 50% table cell)
WIDTH       = 460
LINE_HEIGHT = 22
PADDING_X   = 16
PADDING_Y   = 12
...
# Color palette (matches main terminal)
BG_COLOR     = (13, 17, 23)
...
# Each project: (output_filename, header_title, lines)
PROJECTS = [
    (
...
def get_color(line_type):
    return {
        "cmd": CMD_COLOR, "out": OUT_COLOR,
        "sep": SEP_COLOR, "status": STATUS_COLOR
    }.get(line_type, OUT_COLOR)
...
def draw_frame(lines_data, visible_count, header_title, height, show_cursor=True):
    ...
    if show_cursor and visible_count > 0:
        ...
        try:
            ...
            cx = PADDING_X
        cursor_y = HEADER_HEIGHT + PADDING_Y + (visible_count - 1) * LINE_HEIGHT
        draw.rectangle([cx, cursor_y + 2, cx + 7, cursor_y + LINE_HEIGHT - 3], fill=CURSOR_COLOR)
...
for filename, header_title, lines_data in PROJECTS:
    ...
    for i in range(1, total + 1):
        ...
        lt = lines_data[i - 1][1]
        delay = 700 if lt == "cmd" else 300 if lt == "sep" else 450
    ...
    print(f"Generated: {filename}  ({size_kb} KB, {len(frames)} frames)")
